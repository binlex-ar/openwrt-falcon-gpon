...
#include "me/omci_me_65305.h"

/** \addtogroup OMCI_ME_65305
   @{
*/

/** Structure that implements attr2 table */
struct attr2_list_entry {
	/** Table entry */
	struct omci_me_65305_attr2 table_entry;
	/** Next entry */
	struct attr2_list_entry *next;
	/** Previous entry */
	struct attr2_list_entry *prev;
};

/** Internal data */
struct internal_data {
	/** Multicast Address table (list head) */
	struct attr2_list_entry list_head;
	/** Number of entries in attr2 table */
	size_t entries_num;
};

static enum omci_error attr2_table_entry_set(struct omci_context *context,
					     struct me *me,
					     struct omci_me_65305_attr2 *entry)
{
	enum omci_api_return ret;
	struct internal_data *me_internal_data;
	struct attr2_list_entry *list_entry;
	struct attr2_list_entry *next_list_entry;
	...
	entry-v2 = ntoh16(entry-v2);
	entry-v3 = ntoh16(entry-v3);
#endif

	if (entry->v1 & OMCI_ME_65305_ATTR2_TABLE_ENTRY_OP) {
				   ...
				   struct tbl_copy_entry *tbl_copy)
{
	...
	struct internal_data *me_internal_data;
	struct attr2_list_entry *list_entry;
	struct omci_me_65305_attr2 *tbl_entry;
	...
	switch (attr) {
	case omci_me_65305_attr2:
		tbl_copy->data_size = sizeof(struct omci_me_65305_attr2)
		    * me_internal_data->entries_num;
		...
		tbl_entry = (struct omci_me_65305_attr2 *)tbl_copy->data;
		DLIST_FOR_EACH(list_entry,
			       &me_internal_data->list_head) {
			memcpy(tbl_entry, &list_entry->table_entry,
			       sizeof(tbl_entry));
			tbl_entry->v2 = hton16(tbl_entry->v2);
			tbl_entry->v3 = hton16(tbl_entry->v3);
				 ...
				 uint16_t attr_mask)
{
	...
	struct omci_me_65305 *upd_data;
	struct omci_me_65305 *me_data;

	dbg_in(__func__, "%p, %p, %p, 0x%04x", (void *)context,
	       (void *)me, (void *)data, attr_mask);

	upd_data = (struct omci_me_65305 *) data;
	me_data = (struct omci_me_65305 *) me->data;

	if (attr_mask & omci_attr2mask(omci_me_65305_attr2)) {
		error = attr2_table_entry_set(context, me, &upd_data->attr2);
			       ...
			       uint16_t suppress_avc)
{
	...
	error = me_data_write(context, me, &data, sizeof(data),
			      ...
			      & ~omci_attr2mask(omci_me_65305_attr2),
			      suppress_avc);
	RETURN_IF_ERROR(error);
...
struct me_class me_65305_class = {
	/* Class ID */
	OMCI_ME_65305,
	/* Attributes */
	{
