...
#include "me/omci_gal_ethernet_pmhd.h"
#include "me/omci_api_gal_ethernet_pmhd.h"


#ifdef INCLUDE_PM

/** \addtogroup OMCI_ME_GAL_ETHERNET_PMHD
    @{
*/
				       ...
				       enum omci_pm_interval interval)
{
	...
	struct omci_me_gal_ethernet_pmhd *me_data;
	uint64_t discarded_frames;

	dbg_in(__func__, "%p, %p, %lu", (void *)context, (void *)me, interval);

	me_data = (struct omci_me_gal_ethernet_pmhd *)me->data;

	ret = omci_api_gal_ethernet_pmhd_cnt_get(context->api,
			       me->instance_id,
	...
	me_data->discarded_frames = uint64_lo(discarded_frames);
				  ...
				  uint32_t thr[PM_THR_NUM])
{
	...
	ret = omci_api_gal_ethernet_pmhd_thr_set(context->api, me->instance_id,
						 thr[0]);
	if (ret != OMCI_API_SUCCESS) {
			       ...
			       uint16_t suppress_avc)
{
	...
	/* check if related GEM Interworking TP exists */
...
static struct me_tca tca_table[] = {
	...
	TCA(0, omci_me_gal_ethernet_pmhd_discaded_frames, 1),

	TCA(0, 0, 0)
};

/** Managed Entity class */
struct me_class me_gal_ethernet_class = {
	/* Class ID */
	OMCI_ME_GAL_ETHERNET_PMHD,
	/* Attributes */
	{
		/* 1. Interval End Time */
		ATTR_UINT("Interval end time",
			  ...
			  0x00,
			  0xff,
			  offsetof(struct omci_me_gal_ethernet_pmhd,
				   interval_end_time),
			  1,
			  OMCI_ATTR_PROP_RD,
	...
	tca_table,
	/* Data Size */
	sizeof(struct omci_me_gal_ethernet_pmhd),
	/* Properties */
