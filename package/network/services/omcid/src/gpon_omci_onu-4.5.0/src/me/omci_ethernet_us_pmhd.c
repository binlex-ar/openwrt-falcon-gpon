...
#include "me/omci_ethernet_us_pmhd.h"
#include "me/omci_api_ethernet_us_pmhd.h"


#ifdef INCLUDE_PM

/** \addtogroup OMCI_ME_ETHERNET_PMHD_US
    @{
*/
				       ...
				       enum omci_pm_interval interval)
{
	...
	struct omci_me_ethernet_us_pmhd *me_data;
	...
	me_data = (struct omci_me_ethernet_us_pmhd *)me->data;

	ret = omci_api_ethernet_us_pmhd_cnt_get(context->api, me->instance_id,
						interval ==
	...
	me_data->fcs_error_count = uint64_lo(crc_errored_packets);
	me_data->rx_undersized_packet_count = uint64_lo(undersized_packets);
	me_data->rx_oversized_packet_count = uint64_lo(oversized_packets);
				  ...
				  uint32_t thr[PM_THR_NUM])
{
	...
	ret = omci_api_ethernet_us_pmhd_thr_set(context->api, me->instance_id,
...
static struct me_tca tca_table[] = {
	/* 0 Drop events #1 */
	TCA(0, omci_me_ethernet_us_pmhd_drop_event_count, 1),
	/* 1 CRC errored packets #2 */
	TCA(1, omci_me_ethernet_us_pmhd_fcs_error_count, 2),
	/* 2 Undersize packets #3 */
	TCA(2, omci_me_ethernet_us_pmhd_rx_undersized_packet_count, 3),
	/* 3 Oversize packets #4 */
	TCA(3, omci_me_ethernet_us_pmhd_rx_oversized_packet_count, 4),

	TCA(0, 0, 0)
};

/** Managed Entity class */
struct me_class me_ethernet_us_pmhd_class = {
	/* Class ID */
	OMCI_ME_ETHERNET_FRAME_PMHD_US,
	/* Attributes */
	{
		/* 1. Interval End Time */
		ATTR_UINT("Interval end time",
			  ...
			  0x00,
			  0xff,
			  offsetof(struct omci_me_ethernet_us_pmhd,
				   interval_end_time),
			  1,
			  OMCI_ATTR_PROP_RD,
